...
<template>
    <Head title="Area-wise Sales Report" />
                ...
                <div class="premium-card bg-white rounded-lg shadow-sm">
                    ...
                    >
                        ...
                        <template #cell-orders_count="{ item }">
                            <span class="badge badge-info-soft text-info border px-2">{{ item.orders_count }} Orders</span>
                        </template>
                        <template #cell-total_sales="{ item }">
                            <span class="font-weight-bold text-success">{{ formatMoney(item.total_sales) }}</span>
                        </template>
                        ...
                        <template #footer="{ filteredItems }">
                            <tr class="bg-light d-print-table-row">
                                ...
                                <td class="text-center font-weight-bold py-3">
                                    {{ filteredItems.reduce((sum, a) => sum + Number(a.orders_count || 0), 0) }}
                                </td>
                                <td class="text-right font-weight-bold py-3 text-success">
                                    {{ formatMoney(filteredItems.reduce((sum, a) => sum + Number(a.total_sales || 0), 0)) }}
                                </td>
                            </tr>
                        </template>
                    </PremiumTable>
