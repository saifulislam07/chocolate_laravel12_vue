    ...
    <MainLayout>
                ...
                <section class="mb-10 grid gap-4 lg:grid-cols-[1fr_auto] lg:items-center">
                    <div class="relative">
                        ...
                        <input
                            ...
                            class="h-12 w-full border border-gray-100 bg-white pl-11 pr-4 text-sm shadow-sm transition focus:border-cocov-gold focus:ring-0"
                        />
                    </div>
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                        <button
                            type="button"
                            class="flex h-12 items-center gap-2 border border-gray-100 px-4 text-[10px] font-bold uppercase tracking-widest transition hover:border-cocov-gold lg:hidden"
                            @click="showMobileFilters = true"
                        >
                            <FunnelIcon class="h-4 w-4" />
                            Filters
                            <span v-if="activeFilterCount" class="rounded-full bg-cocov-gold px-2 py-0.5 text-white">{{ activeFilterCount }}</span>
                        </button>

                        <select
                            v-model="filterForm.sort"
                            class="h-12 border border-gray-100 bg-white px-4 text-[10px] font-bold uppercase tracking-widest shadow-sm focus:border-cocov-gold focus:ring-0"
                        >
                        ...
                        <div class="sticky top-24 space-y-10">
                            <section>
                                <div class="mb-5 flex items-center justify-between">
                                    <h3 class="text-[11px] font-bold uppercase tracking-[0.2em] text-cocov-text">Collections</h3>
                                ...
                                <div class="space-y-2">
                                    <button
                                        ...
                                        :class="filterForm.category === cat.slug ? 'border-cocov-gold bg-[#fcf8f3] text-cocov-text' : 'border-gray-100 text-gray-500 hover:border-gray-200 hover:text-cocov-text'"
                            ...
                            <section>
                                <h3 class="mb-5 text-[11px] font-bold uppercase tracking-[0.2em] text-cocov-text">Price</h3>
                                <div class="grid grid-cols-2 gap-3">
                                    <input v-model="filterForm.min_price" type="number" placeholder="Min" class="h-11 border-gray-100 text-xs focus:border-cocov-gold focus:ring-0" />
                                    <input v-model="filterForm.max_price" type="number" placeholder="Max" class="h-11 border-gray-100 text-xs focus:border-cocov-gold focus:ring-0" />
                                </div>
                                <div class="mt-4 flex flex-wrap gap-2">
                                    <button type="button" class="border border-gray-100 px-3 py-2 text-[10px] font-bold uppercase tracking-widest hover:border-cocov-gold" @click="setPriceRange('', 50)">Under 50</button>
                                    <button type="button" class="border border-gray-100 px-3 py-2 text-[10px] font-bold uppercase tracking-widest hover:border-cocov-gold" @click="setPriceRange(50, 100)">50 - 100</button>
                                    <button type="button" class="border border-gray-100 px-3 py-2 text-[10px] font-bold uppercase tracking-widest hover:border-cocov-gold" @click="setPriceRange(100, '')">100+</button>
                                </div>
                            ...
                            <section class="space-y-3">
                                <label class="flex cursor-pointer items-center justify-between border border-gray-100 px-4 py-3 text-xs">
                                    ...
                                    <input v-model="filterForm.sale" type="checkbox" class="rounded border-gray-300 text-cocov-gold focus:ring-cocov-gold" />
                                </label>
                                <label class="flex cursor-pointer items-center justify-between border border-gray-100 px-4 py-3 text-xs">
                                    <span>In stock only</span>
                                    <input v-model="filterForm.in_stock" type="checkbox" class="rounded border-gray-300 text-cocov-gold focus:ring-cocov-gold" />
                                </label>
                            </section>
                        </div>
                    </aside>
                    ...
                    <section class="min-w-0 flex-1">
                        <div class="mb-8 flex flex-col gap-3 border-b border-gray-100 pb-5 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <div v-if="activeFilterCount" class="flex flex-wrap gap-2">
                                <span class="bg-[#fcf8f3] px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-cocov-text">
                        ...
                        <div v-if="displayedProducts.length" class="grid grid-cols-1 gap-x-8 gap-y-12 sm:grid-cols-2 xl:grid-cols-3">
                            <article v-for="product in displayedProducts" :key="product.id" class="group relative flex flex-col bg-white">
                                ...
                                <div class="flex flex-1 flex-col p-6 text-center">
                                    <h3 class="font-heading text-lg uppercase leading-tight tracking-tight">
                        ...
                        <div v-else class="rounded-sm border border-dashed border-gray-200 py-24 text-center">
                            <p class="font-heading text-2xl uppercase text-gray-400">No treasures found matching your search</p>
                            <button type="button" class="mt-6 text-[10px] font-bold uppercase tracking-[0.2em] text-cocov-gold hover:underline" @click="resetFilters">Clear all filters</button>
                        </div>

                        <div v-if="hasMoreProducts" class="mt-16 flex justify-center border-t border-gray-100 pt-10">
                            <button
                                type="button"
                                class="min-w-48 rounded-[3px] border border-cocov-gold px-10 py-4 text-[11px] font-bold uppercase tracking-[0.2em] text-cocov-gold transition hover:bg-cocov-gold hover:text-white disabled:cursor-wait disabled:opacity-50"
        ...
        <div v-if="showMobileFilters" class="fixed inset-0 z-[100] overflow-y-auto bg-white lg:hidden">
            ...
            <div class="space-y-8 p-6">
                <section>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <button
                            ...
                            :class="filterForm.category === cat.slug ? 'bg-cocov-gold text-white' : 'border border-gray-100 bg-white'"
                            @click="setCategory(cat.slug)"
                        >
                            {{ cat.name }}
                        </button>
                ...
                <button class="w-full bg-cocov-gold py-5 text-[11px] font-bold uppercase tracking-[0.3em] text-white transition hover:bg-[#e0851a]" @click="applyFilters(); showMobileFilters = false">
                    Apply Filters
                </button>
